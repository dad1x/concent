...
class SCIBaseTest():
    ...
    def request_for_deposit(self, value: int=0):
        ...
        self._wait_unitl_timeout(
            condition=lambda: (
                self.get_requestor_deposit_value() == 0 or
                self.get_provider_deposit_value() == 0
            ),
    ...
    def get_requestor_deposit_value(self):
        return self.requestor_sci.get_deposit_value(self.requestor_sci.get_eth_address())

    def get_provider_deposit_value(self):
        return self.provider_sci.get_deposit_value(self.provider_sci.get_eth_address())

    def ensure_that_provider_has_specific_gntb_balance(self, value):
        self._wait_unitl_timeout(
            condition=lambda: (
                    self.get_provider_gntb_balance() != value
            ),
    ...
    def ensure_that_requestor_has_specific_deposit_balance(self, value):
        self._wait_unitl_timeout(
            condition=lambda: (
                    self.get_requestor_deposit_value() != value
            ),
